...
from desktop_app.domain.artifact_effects import artifact_effect_is_legacy, ARTIFACT_EFFECT_IDS_BY_ARTIFACT_TYPE
...
def optimize_greedy(account: AccountData, presets: BuildStore, req: GreedyRequest) -> GreedyResult:
    ...
    if profile == "gpu_combo":
        from desktop_app.engine.gpu_combo_optimizer import optimize_gpu_combo

        return optimize_gpu_combo(account, presets, req)
    if profile == "max_quality":
        from desktop_app.engine.global_optimizer import optimize_global
    ...
    for idx, unit_ids in enumerate(pass_orders):
        ...
        if use_refine:
            from desktop_app.engine.refine_optimizer import run_refine_pass
